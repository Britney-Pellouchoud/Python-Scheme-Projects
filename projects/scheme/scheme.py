...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if rest is nil: 
            return scheme_apply(func, nil, env)
        helper = lambda x: scheme_eval(x, env)
        mapped = Pair.map(rest, helper)
        return scheme_apply(func, mapped, env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def helper(formals, vals):
            if formals is nil and vals is nil:
                return new_frame
            elif formals.second is nil and vals.second is nil:
                new_frame.define(formals.first, vals.first)
                return new_frame
            elif formals.second is not nil and vals.second is not nil:
                new_frame.define(formals.first, vals.first)
                return helper(formals.second, vals.second)
            else:
                raise SchemeError('Variable and values do not match up.')
        return helper(formals, vals)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Wrong number of arguments were passed in")
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        new = Frame(self.env).make_child_frame(self.formals, args)
        return new
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        body = target.second
        function = do_lambda_form(Pair(body, expressions.second), env)
        env.define(func_name, function)
        return func_name
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    #print("**************", bindings.first.first, bindings.first.second.first)
    ...
    while bindings is not nil:
        ...
        #if isinstance(bindings.first.first, int):
        #    return SchemeError
        child_frame.define(bindings.first.first, scheme_eval(bindings.first.second.first, env))
...
class MuProcedure(Procedure):
    ...
    # BEGIN PROBLEM 16
    def make_call_frame(self, args, env):
        #this_environment = Frame(self.env)
        #print("**************", "THIS HITS", "****************")
        #child_frame = Frame().make_child_frame(self.formals, args)
        return Frame(env).make_child_frame(self.formals, args)
    # END PROBLEM 16
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    #print("**************", "THIS HITS", "****************")
    #LambdaProcedure(formals, expressions.second, env)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        if not tail:
            ...
            return optimized_eval(result, result.env)
        # END
